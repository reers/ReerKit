...
final class FileManagerExtensionsTests: XCTestCase {
    func testJSONFromFileAtPath() {
        ...
        do {
            ...
            let json = try FileManager.default.re.jsonFromFile(atPath: path)
    ...
    func testJSONFromFileWithFilename() {
        ...
        do {
            ...
            var json = try FileManager.default.re.jsonFromFile(withFilename: filename, at: FileManagerExtensionsTests.self)

            XCTAssertNotNil(json)

            filename = "test" // Without extension
            json = try FileManager.default.re.jsonFromFile(withFilename: filename, at: FileManagerExtensionsTests.self)
    ...
    func testInvalidFile() {
        ...
        do {
            let json = try FileManager.default.re.jsonFromFile(withFilename: filename, at: FileManagerExtensionsTests.self)
            XCTAssertNil(json)
        } catch {}
        #endif
    }
    ...
    func testCreateTemporaryDirectory() {
        do {
            ...
            let tempDirectory = try fileManager.re.createTemporaryDirectory()
