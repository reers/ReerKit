...
//  ReerKit-iOSTests
//
//  Created by phoenix on 2022/12/16.
...
final class CGVectorExtensionsTests: XCTestCase {
    func testAngle() {
        ...
        XCTAssertEqual(vector1.re.angle, .pi / 4)
        XCTAssertEqual(vector2.re.angle, 0)
        XCTAssertEqual(vector3.re.angle, .pi / 2)
        XCTAssertEqual(vector4.re.angle, .pi)
        XCTAssertEqual(vector5.re.angle, -(.pi / 2))
        XCTAssertEqual(vector6.re.angle, -(3 * .pi / 4))
        XCTAssertEqual(vector7.re.angle, -(.pi / 4))
        XCTAssertEqual(vector8.re.angle, 3 * .pi / 4)
    }
    ...
    func testMagnitude() {
        ...
        XCTAssertEqual(vector1.re.magnitude, 5)
        XCTAssertEqual(vector2.re.magnitude, sqrt(2))
        XCTAssertEqual(vector3.re.magnitude, 20)
        XCTAssertEqual(vector4.re.magnitude, 13)
        XCTAssertEqual(vector5.re.magnitude, 17)
    }
    ...
    func testInitWithAngleAndMagnitude() {
        let vector1 = CGVector.re(angle: .pi / 4, magnitude: sqrt(2))
        let vector2 = CGVector.re(angle: .pi, magnitude: 1)
        let vector3 = CGVector.re(angle: .pi / 6, magnitude: 2)
        let vector4 = CGVector.re(angle: .pi / 3, magnitude: 2)
