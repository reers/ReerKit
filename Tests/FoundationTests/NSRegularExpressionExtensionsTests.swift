...
final class NSRegularExpressionExtensionsTests: XCTestCase {
    ...
    func testEnumerateMatches() {
        var count = 0
        regularExpression.re.enumerateMatches(in: string,
                                           options: [],
    ...
    func testMatches() {
        let matches = regularExpression.re.matches(in: string,
                                                options: [],
    ...
    func testNumberOfMatches() {
        XCTAssertEqual(regularExpression.re.numberOfMatches(in: string,
                                                         options: [],
                                                         range: string.startIndex..<string.endIndex),
                       expectedMatches)
    }
    ...
    func testFirstMatch() {
        let firstMatch = regularExpression.re.firstMatch(in: string,
                                                      options: [],
                                                      range: string.startIndex..<string.endIndex)
    ...
    func testRangeOfFirstMatch() {
        let range = regularExpression.re.rangeOfFirstMatch(in: string,
                                                        options: [],
                                                        range: string.startIndex..<string.endIndex)
        ...
        let foo = "foo"
        XCTAssertNil(regularExpression.re.rangeOfFirstMatch(in: foo, options: [], range: foo.startIndex..<foo.endIndex))
    }

    func testStringByReplacingMatches() {
        let newString = regularExpression.re.stringByReplacingMatches(in: string,
                                                                   options: [],
                                                                   range: string.startIndex..<string.endIndex,
                                                                   withTemplate: "$0$0")
    ...
    func testReplaceMatches() {
        ...
        let matches = regularExpression.re.replaceMatches(in: &newString,
                                                       options: [],
                                                       range: string.startIndex..<string.endIndex,
                                                       withTemplate: "$0$0")
        XCTAssertEqual(newString, "barbar foo barbar foo barbar")
