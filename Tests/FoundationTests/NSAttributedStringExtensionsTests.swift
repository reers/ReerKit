...
final class NSAttributedStringExtensionsTests: XCTestCase {
    func testBolded() {
        #if !os(Linux)
        let unsizedAttributes = NSAttributedString(string: "Bolded").re.bolded.re.attributes
        XCTAssertEqual((unsizedAttributes[.font] as? REFont)?.fontName, REFont.boldSystemFont(ofSize: 1).fontName)

        let sizedAttributes = NSAttributedString(string: "Bolded", attributes: [.font: REFont.systemFont(ofSize: 12)]).re.bolded.re.attributes
        XCTAssertEqual((sizedAttributes[.font] as? REFont), REFont.boldSystemFont(ofSize: 12))
        #endif
    }

    func testUnderlined() {
        #if !os(Linux)
        let attributes = NSAttributedString(string: "Underlined").re.underlined.re.attributes
    ...
    func testItalicized() {
        #if canImport(UIKit)
        let unsizedAttributes = NSAttributedString(string: "Italicized").re.italicized.re.attributes
        XCTAssertEqual((unsizedAttributes[.font] as? UIFont)?.fontName, UIFont.italicSystemFont(ofSize: 1).fontName)

        let sizedAttributes = NSAttributedString(string: "Italicized", attributes: [.font: REFont.systemFont(ofSize: 12)]).re.italicized.re.attributes
    ...
    func testStruckthrough() {
        ...
        let attributes = NSAttributedString(string: "Struck through").re.struckthrough.re.attributes
    ...
    func testColored() {
        ...
        var out = string.re.colored(with: .red)
        var attributes = out.attributes(at: 0, effectiveRange: nil)
        let filteredAttributes = attributes.filter { (key, value) -> Bool in
            return (key == NSAttributedString.Key.foregroundColor && (value as? REColor) == .red)
        }

        XCTAssertEqual(filteredAttributes.count, 1)

        out = out.re.colored(with: .blue)
        attributes = out.attributes(at: 0, effectiveRange: nil)
        XCTAssertEqual(attributes[NSAttributedString.Key.foregroundColor] as? REColor, .blue)
        XCTAssertNotEqual(attributes[NSAttributedString.Key.foregroundColor] as? REColor, .red)
        #endif
    }
    ...
    func testApplyingToRegex() {
        ...
        let testString = NSAttributedString(string: "Your email is \(email)!").re.bolded
    ...
    private func caseSensitiveRegexTest(
        ...
        let caseInsensitiveAttrString = stringToTest.re.applying(
        ...
        let caseSensitiveAttrString = stringToTest.re.applying(attributes: attributes, toRangesMatching: pattern)
    ...
    private func commonRegexTest(
        ...
        let attrTestString = stringToTest.re.applying(attributes: attributes, toRangesMatching: pattern)
    ...
    func testApplyingToOccurrences() {
        ...
        let attrGreeting = NSAttributedString(string: greeting).re.italicized.re.applying(
    ...
    func testAppending() {
        #if os(iOS)
        var string = NSAttributedString(string: "Test").re.italicized.re.underlined.re.struckthrough
        string += NSAttributedString(string: " Appending").re.bolded
    ...
    func testAttributes() {
        #if os(iOS)
        let emptyString = NSAttributedString(string: "").re.bolded.re.struckthrough.re.underlined.re.colored(with: UIColor.blue)
        let emptyStringAttributes = emptyString.re.attributes
        XCTAssert(emptyStringAttributes.isEmpty)

        let attrString = NSAttributedString(string: "Test String")
            .re.bolded
            .re.struckthrough
            .re.underlined
            .re.colored(with: UIColor.blue)
        let attributes = attrString.re.attributes
    ...
    func testOperators() {
        #if os(iOS)
        var string1 = NSAttributedString(string: "Test").re.italicized.re.underlined.re.struckthrough
        let string2 = NSAttributedString(string: " Appending").re.bolded
